...
STOCK_SCHEMA = ParamSchema(fields=[
    ...
    ParamField(key="MIN_CONFIDENCE", label="Min confidence", type="percent",
               default=0.60, min=0.0, max=1.0, step=0.01,
               help="Reject Claude signals below this confidence."),
    ParamField(key="WATCHLIST", label="Watchlist (comma-separated)", type="text",
               default="AAPL,TSLA,NVDA,MSFT,AMZN,GOOGL,META,NFLX,AMD,JPM,V,UBER,PLTR",
               help="Tickers the bot may trade."),
    # Step 2 — Sizing
    ParamField(key="MAX_POSITION_SIZE_PCT", label="Max position size", type="percent",
    ...
    ParamField(key="TAKE_PROFIT_PCT", label="Take profit", type="percent",
               default=0.04, min=0.005, max=1.0, step=0.005,
               help="Close a position once it gains this far from entry."),
    # Step 4 — AI & universe
    ParamField(key="CLAUDE_MODEL", label="Claude model", type="select",
               default="claude-sonnet-4-6", options=_MODEL_OPTIONS,
               help="Which Claude model generates signals for this strategy."),
    ParamField(key="ENABLE_SCREENER", label="Enable dynamic screener", type="bool",
               default=False,
               help="Let the bot add screener-discovered tickers beyond the watchlist."),
    ParamField(key="MAX_SCREENER_ADDITIONS", label="Max screener additions", type="number",
               default=3.0, min=0.0, max=25.0, step=1.0,
               help="Cap on tickers the screener may add per cycle."),
    ParamField(key="BLOCK_NEW_POSITIONS_ON_EARNINGS", label="Block around earnings", type="bool",
               default=True,
               help="Skip opening positions inside a ticker's earnings window."),
    ParamField(key="BLOCK_NEW_POSITIONS_ON_MACRO", label="Block around macro events", type="bool",
               default=True,
